...
import { catalogFromLiveAgents, createSession, loadCatalog, truncateButton, visibleSessionsForWorkspace, workspacesWithVisibleSessions, } from './catalog.js';
...
export class TelegramBridge {
    ...
    async sendWorkspacePicker(chatId, existing, mode = 'attach') {
    ...
    async createNewBound(chatId, callbackId, workspace) {
    ...
    startTypingKeepalive(chatId) {
        ...
        const tick = () => {
            if (!this.typingTimers.has(key))
                return; // disarmed between schedule and fire
    ...
    async onSessionEvent(session, event) {
        ...
        if (event.type === 'turn/start') {
            ...
            for (const b of targets)
                this.startTypingKeepalive(b.chatId);
            return;
        }
        ...
        if (event.type === 'turn/end') {
            for (const b of targets)
                this.stopTypingKeepalive(b.chatId);
            return;
        }
