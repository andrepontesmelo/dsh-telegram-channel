...
param(
  ...
  [string] $Source = 'github:andrepontesmelo/dsh-telegram-channel',
  [int] $Port = 3080,
  [switch] $Local,
  [switch] $NoPersist
)
...
function Ensure-AllowBuilds([string] $ProfileDir) {
  ...
  $repoKeyPresent = $raw -match 'dsh-telegram-channel@git\+https://github\.com/andrepontesmelo/dsh-telegram-channel\.git'
  ...
  if (-not $repoKeyPresent) {
    $toInsert += "  'dsh-telegram-channel@git+https://github.com/andrepontesmelo/dsh-telegram-channel.git': true"
  }
